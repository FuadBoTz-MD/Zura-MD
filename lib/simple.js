...
END:VCARD
   ...
   /**
     ...
     * @param {Object} options 
     */
    sendContactArray: { async value (jid, data, quoted, options) {
        let contacts = []
        ...
        hydratedTemplate: {
          ...
          hydratedButtons: [{
            urlButton: {
              ...
              url: 'https://www.instagram.com/_alv.stn'
            }
          },
          {
            quickReplyButton: {
 ...
 */
sendVinButtonLoc: {
    async value(jid, buffer, contentText, footer, url1, url2, buttons1, row1, quoted, options) {
    const type = await conn.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
...
sendVin2ButtonLoc: {
    async value(jid, buffer, contentText, footer, url1, url2, buttons1, row1, buttons2, row2, quoted, options) {
    const type = await conn.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
...
sendVin3ButtonLoc: {
    async value(jid, buffer, contentText, footer, url1, url2, buttons1, row1, buttons2, row2, buttons3, row3, quoted, options) {
    const type = await conn.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
...
sendTemplateButtonFakeImg: {
    async value(jid, buffer, content, footerText, btn1, id1, options) {
        ...
        const key = {
            ...
            ...options,
            templateButtons: [
            { index: 1, urlButton: { displayText: 'Instagram', url: 'https://www.instagram.com/_alv.stn' } },
            { index: 2, callButton: { displayText: 'Call Owner',  phoneNumber: '6287734910547' } },
...
sendTBlist: {
    async value(jid, buffer, content, footerText, button1, id1, quoted, options) {
        const buttons = [
            { index: 1, urlButton: { displayText: 'Chat Owner', url: 'https://wa.me/6287734910547?text=📑Bang+mau+beli+jasa+enc+fitur' } },
            { index: 2, quickReplyButton: { displayText: button1, id: id1 } },
        ]
        const buttonMessage = {
            document: { url: "https://wa.me/6287734910547"},
            mimetype: doc,
            fileName: ucapan,
            fileLength: 2673838289,
            pageCount: 1001,
            caption: content,
            footer: footerText,
            mentions: await conn.parseMention(content + footerText),
     ...
     */
        ...
        sendButtonDoc: {   
            ...
            async value(jid, content, footerText, button1, id1, quoted, options) {
              ...
              conn.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: ephemeral, forwardingScore: 84600, isForwarded: true, ...options })
            },
            enumerable: true
        },
        send2ButtonDoc: {
        ...
        conn.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: ephemeral, contextInfo: { mentionedJid: conn.parseMention(content + footerText), forwardingScore: 84600, isForwarded: true }, ...options, ephemeralExpiration: ephemeral } )
    },
    enumerable: true
},
        sendHydrated: {
