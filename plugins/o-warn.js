let handler = async (m, { conn, args }) => {
    ...
    if (user.Banneduser) throw '⚠️User telah terbanned!!'
    if ((user.warn * 1) < 3) {
        user.warn += 1
        m.reply('✔️berhasil Warn')
        m.reply('📮Kamu di warn oleh OWNER!!, dan sekarang kamu punya *' + (user.warn + 1) + '* WARN. Ingat Jika kamu mendapat warn 4 kali kamu akan otomatis ke banned', mention)
