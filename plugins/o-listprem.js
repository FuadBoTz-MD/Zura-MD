let handler = async (m, { conn, args }) => {
  ...
  let kled = 'https://telegra.ph/file/9c8fe8cb7dd7334612ed9.png'
  let premTime = global.db.data.users[m.sender].premiumTime
  let prem = global.db.data.users[m.sender].premium
  let waktu = clockString(`${premTime - new Date() * 1} `)
  let sortedP = user.map(toNumber('premiumTime')).sort(sort('premiumTime'))
  let len = args[0] && args[0].length > 0 ? Math.min(100, Math.max(parseInt(args[0]), 10)) : Math.min(10, sortedP.length)
  let info= `┌✦ *My Premium Time:*
...
┗━═┅═━––––––๑`.trim()
await conn.reply(m.chat, info, m, { contextInfo: { isForwarded: false, forwardingScore: 9999, externalAdReply :{ mediaType: 1, mediaUrl: kled, title: `]======❏ Premium ❏======[`, thumbnail: { url: kled }, thumbnailUrl: kled, renderLargerThumbnail: true }}})
setTimeout(() => {
