let handler = async (m, { conn, isAdmin, isOwner, args, usedPrefix, command }) => {
	...
	if (!isOwner) {
	  ...
	  await conn.groupSettingUpdate(args[0], 'not_announcement').then(async _=> {
	  m.reply(`Sukses membuka grup`)
  }).then(async _=> {conn.reply(args[0], `Owner telah membuka group ini`)
  })
}
handler.command = /^(timeopen)$/i
